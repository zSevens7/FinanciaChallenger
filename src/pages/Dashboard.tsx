...
const Dashboard = () => {
  ...
  const formattedTransactions: Transaction[] = useMemo(() => {
    ...
    return metrics.transactions.map(tx => ({
      id: String(tx.id ?? crypto.randomUUID()),   // garante string
      data: tx.data ?? "",
      type: tx.type === "Venda" ? "sale" : "expense", // mapeia corretamente
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-4 text-sm">
          {kpis.map(kpi => (
            <DashboardKPI
              key={kpi.name}
              title={kpi.name}
              value={typeof kpi.value === "number" ? kpi.value.toFixed(2) : kpi.value}
              period={
