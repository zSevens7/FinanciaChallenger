...
export interface VendasTableProps {
  vendas: Venda[];
  onDelete?: (id: string) => void; // id é string
}
...
export const VendasTable = ({ vendas, onDelete }: VendasTableProps) => {
  ...
  const totalColumns = 4; // Data, Descrição (desktop), Valor, Ação
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-green-600 text-white">
              <tr>
                ...
                <th className={`p-3 text-left font-semibold tracking-wider whitespace-nowrap hidden sm:table-cell`}>
                  Descrição
                </th>
              ...
              {vendas.length > 0 ? (
                vendas.map((venda) => (
                  ...
                  >
                    ...
                    <td className={`p-3 whitespace-nowrap text-gray-700 hidden sm:table-cell`}>
                      {venda.descricao ?? '-'}
                    </td>
