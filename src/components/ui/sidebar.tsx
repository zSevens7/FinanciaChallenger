...
import { PanelLeftIcon, X, Menu } from "lucide-react"; // Importe X e Menu aqui também para o SidebarTrigger
...
export function SidebarProvider({
  ...
  const setOpen = (value: boolean) => {
    if (isMobile) {
      setOpenMobileInternal(value);
    } else {
      setOpenStateInternal(value);
    }
  };
...
function Sidebar({
  ...
  const { isMobile, state, open: contextOpen, setOpen } = useSidebar();
    ...
    >
      ...
      >
        <div
          ...
          className="bg-purple-700 group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm"
        >
          {children}
        </div>
      </div>
